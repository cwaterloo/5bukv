...
namespace FiveLetters
{
    internal sealed class TreeGenerator
    {
        ...
        private TreeGenerator(
            ...
            IReadOnlyList<Word> hardcodedWords,
        ...
        internal static Tree Get(
            IReadOnlyList<string> globalWordStrings,
            IReadOnlyList<string> attackWordStrings,
            IReadOnlyList<string> hardcodedWords,
            bool dual
        )
        {
            (
                List<Word> globalWords,
                List<Word> attackWords,
                List<Word> hardcoded,
                List<char> alphabet
            ) = GetWords(globalWordStrings, attackWordStrings, hardcodedWords.Distinct().ToList());
        ...
        )
        {
            if (hardcodedWords.Except(attackWordStrings).Any())
            {
                throw new InvalidOperationException(
                    "Hardcoded word list is not a subset of attack word list."
                );
            }
            ...
            List<Word> globalWords =
            [
                .. globalWordStrings.Select(word => ToWord(word, charToLetterMap)),
            ];
            List<Word> attackWords =
            [
                .. attackWordStrings.Select(word => ToWord(word, charToLetterMap)),
            ];
            Console.WriteLine(
                "Loaded {0} global words and {1} attack words.",
                globalWords.Count,
                attackWords.Count
            );
            ...
            return (
                globalWords,
                attackWords,
                [.. hardcodedWords.Select(word => ToWord(word, charToLetterMap))],
                alphabet
            );
        }
        ...
        private Tree Make(IReadOnlyList<Word> candidates, int level)
        {
            ...
            foreach (Word hiddenWord in candidates)
            {
                ...
                List<Word> words = stateWords.TryGetValue(packedState, out List<Word>? value)
                    ? value
                    : stateWords[packedState] = [];
                words.Add(hiddenWord);
            }

            Dictionary<int, Tree> edges =
                stateWords.Count == 1 && stateWords.Single().Value.Count == 1
                    ? []
                    : stateWords.ToDictionary(
                        keyValue => keyValue.Key,
                        keyValue => Make(keyValue.Value, level + 1)
                    );
            return new() { Word = ToString(guess), Edges = { edges } };
        }

        private Tree Make2(IReadOnlyList<Word> candidates, int level)
        {
            (Word firstGuess, Word secondGuess) = AI.GetCandidate2(
                candidates,
                attackWords,
                alphabet.Count
            );
            ...
            foreach (Word hiddenWord in candidates)
            {
                int firstPackedState = new Evaluation(
                    ToString(firstGuess),
                    ToString(hiddenWord)
                ).Pack();
                Dictionary<int, List<Word>> firstWords = stateWords.TryGetValue(
                    firstPackedState,
                    out Dictionary<int, List<Word>>? value1
                )
                    ? value1
                    : stateWords[firstPackedState] = [];
                int secondPackedState = new Evaluation(
                    ToString(secondGuess),
                    ToString(hiddenWord)
                ).Pack();
                List<Word> secondWords = firstWords.TryGetValue(
                    secondPackedState,
                    out List<Word>? value2
                )
                    ? value2
                    : firstWords[secondPackedState] = [];
            ...
            foreach (KeyValuePair<int, Dictionary<int, List<Word>>> firstKeyValue in stateWords)
            {
                ...
                edges.Add(
                    ...
                    new Tree
                    {
                        ...
                        Edges =
                        {
                            firstKeyValue.Value.ToDictionary(
                                secondKeyValue => secondKeyValue.Key,
            ...
            return new() { Word = ToString(firstGuess), Edges = { edges } };
        }
